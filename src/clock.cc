...
uint64_t RTCP::clock::ntp::now()
{
    struct timeval tv;
#ifdef _WIN32
    RTCP::clock::gettimeofday(&tv, NULL);
#else
...
uint64_t RTCP::clock::ntp::diff(uint64_t ntp1, uint64_t ntp2)
{
    return ntp_diff_ms(ntp1, ntp2);
}

uint64_t RTCP::clock::ntp::diff_now(uint64_t then)
{
    uint64_t now = RTCP::clock::ntp::now();

    return ntp_diff_ms(then, now);
}

RTCP::clock::hrc::hrc_t RTCP::clock::hrc::now()
{
    return std::chrono::high_resolution_clock::now();
}

uint64_t RTCP::clock::hrc::diff(hrc_t hrc1, hrc_t hrc2)
{
    return std::chrono::duration_cast<std::chrono::milliseconds>(hrc1 - hrc2).count();
}

uint64_t RTCP::clock::hrc::diff_now(hrc_t then)
{
...
uint64_t RTCP::clock::hrc::diff_now_us(hrc_t& then)
{
...
uint64_t RTCP::clock::ms_to_jiffies(uint64_t ms)
{
    return (uint64_t)(((double)ms/1000)* 65536);
}

uint64_t RTCP::clock::jiffies_to_ms(uint64_t jiffies)
{
    return (uint64_t)(((double)jiffies / 65536) * 1000);
}

#ifdef _WIN32
int RTCP::clock::gettimeofday(struct timeval * tp, struct timezone * tzp)
{
